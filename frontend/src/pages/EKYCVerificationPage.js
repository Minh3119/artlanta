...
const EKYCVerificationPage = () => {
  ...
  const handleSubmit = async () => {
    if (!documentImage || !faceImage) {
      setErrors({
        general: `Thiếu ảnh ${
          documentType === "cccd" ? "CCCD" : "Passport"
        } hoặc ảnh chân dung.`,
      });
      return;
    }
    ...
    try {
      const response = await fetch(
        "http://localhost:9999/backend/api/verify-identity",
        {
          method: "POST",
          body: formData,
          credentials: "include",
      ...
      if (response.ok && data.success) {
        ...
        let errorMessage =
          data.message || "Có lỗi xảy ra trong quá trình xác thực";
  ...
  const FileUploadArea = ({
    ...
    <div className="upload-area">
      ...
      <div
        className={`upload-zone ${error ? "error" : ""} ${
          preview ? "success" : ""
        }`}
    ...
    <div className="ekyc-container">
      <div className="ekyc-wrapper">
        <div className="ekyc-header">
          ...
          <p className="ekyc-subtitle">
            Tải lên ảnh giấy tờ tùy thân và ảnh chân dung để xác thực danh tính
          </p>
        </div>
        ...
        <div className="ekyc-card">
          ...
          <div className="upload-grid">
            <FileUploadArea
              ...
              title={`Ảnh ${
                documentType === "cccd" ? "CCCD/CMND" : "Passport"
              }`}
          ...
          <div className="instructions">
            ...
            <ul className="instructions-list">
              <li>
                •{" "}
                <strong>
                  Ảnh {documentType === "cccd" ? "CCCD/CMND" : "Passport"}:
                </strong>{" "}
                Chụp rõ nét, đầy đủ 4 góc, không bị che khuất hoặc phản quang
              </li>
              <li>
                • <strong>Ảnh chân dung:</strong> Nhìn thẳng vào camera, đủ ánh
                sáng, không đeo khẩu trang
              </li>
              <li>
                • <strong>Chất lượng:</strong> Cả hai ảnh phải có độ phân giải
                cao và không bị mờ
              </li>
              <li>
                • <strong>Định dạng:</strong> Chỉ chấp nhận file JPEG/PNG, tối
                đa 5MB mỗi file
              </li>
              {documentType === "passport" && (
                <li>
                  • <strong>Passport:</strong> Chụp trang có thông tin cá nhân
                  và ảnh chân dung
                </li>
              )}
            </ul>
