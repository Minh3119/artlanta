...
export const AvatarPlaceholder = ({ size = 'md' }) => {
    ...
    return (
        <div className={`${sizeClasses[size]} rounded-full bg-gray-200 flex items-center justify-center border-4 border-white overflow-hidden`}>
...
const AvatarImage = ({ avatarUrl, displayName, size = 'md' }) => {
    ...
    return (
        <img
            src={avatarUrl}
            alt={displayName}
            className={`${sizeClasses[size]} rounded-full border-4 border-white overflow-hidden object-cover bg-gray-100`}
