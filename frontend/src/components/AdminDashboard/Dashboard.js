...
export default function Dashboard() {
  const [users, setUsers] = useState({ raw: [], chart_data: [], total_user: 0, mostUserCreDay: "" });
...
<div>
      ...
      <div style={{ minHeight: '500px', maxHeight: '500px', overflowY: 'auto', marginLeft:'30px' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
          {filteredUsers
  .filter(user => user.role !== 'CLIENT' && user.role !== 'ARTIST')
  .map((user, index) => (
              <tr key={index}>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <section className="card metrics">
          ...
          <div className="card__content metrics">
            <div className="metric">
              ...
              <div>
                <strong>12%</strong><br />Total Post
              </div>
            </div>
            <div className="metric">
              <i className="metric__icon warning"></i>
              <div>
                <strong>0.8%</strong><br />Post-to-Engagement Ratio (PER)
              </div>
            </div>
            <div className="metric">
              <i className="metric__icon down"></i>
              <div>
                <strong>1%</strong><br />Average Comments per Post
              </div>
            </div>
          </div>
