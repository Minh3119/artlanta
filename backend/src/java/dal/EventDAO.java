...
public class EventDAO extends DBContext {
     ...
     */
    public List<Post> getEventPosts(int eventId) {
        String sql = "SELECT p.* FROM Posts p " +
                    "JOIN event_posts ep ON p.ID = ep.post_id " +
                    "WHERE ep.event_id = ? " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                post.setIsDraft(rs.getBoolean("IsDraft"));
            ...
            System.out.println("Error getting event posts: " + e.getMessage());
        }
    ...
    public Event getEventWithFollowers(int eventId) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // Get event posts
                event.setPosts(getEventPosts(eventId));
